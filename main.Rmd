...
# train the model
rf <- train(taxonID~., data=input, trControl=train_control, method="rf")
...
# HEIGHT
ggplot(aes(taxonID, height, fill = taxonID), data = input) +
  geom_boxplot() + 
  ggtitle("Height distributions per species") + 
  labs(x = "Species ID", y = "Height (m)") + 
  scale_fill_discrete(name = "Species ID")
# CROWN DIAMETER
ggplot(aes(taxonID, maxCrownDiameter, fill = taxonID), data = input) +
  geom_boxplot() + 
  ggtitle("Crown Diameter distributions per species") + 
  labs(x = "Species ID", y = "Max Crown Diameter (m)") + 
  scale_fill_discrete(name = "Species ID")
# SHAPE 
shapes <- input$shape[input$taxonID=="CELE2"]
input %>% group_by(shape) %>% 
  ggplot(aes(x = taxonID, y = shape, fill = shape)) + 
    geom_bar(stat = "identity") 

i <- input %>% group_by(taxonID, shape) %>% summarize(total = n())
shapeTable <- xtabs(total~shape+taxonID, data=i)
datm <- melt(cbind(shapeTable, ind = rownames(shapeTable)), id.vars = c('ind'))
ggplot(data = i, aes(x=taxonID, y=shape, fill = shape)) + 
  geom_bar(stat = "identity")
...
model2.2 = model2$pred[seq(1, nrow(model2$pred), 3), ]

cm <- data.frame(melt(xtabs(~ model2.2$pred + model2.2$obs)))

#confusion_matrix <- as.data.frame(table(model2.2$pred, model2.2$obs))
ggplot(data = cm,
       mapping = aes(x = model2.2.pred,
                     y = model2.2.obs)) +
  geom_tile(aes(fill = value)) +
  geom_text(aes(label = sprintf("%1.0f", value)), vjust = 1) +
  scale_fill_gradient(low = "gray",
...
confusionMatrix(model2.2$pred,model2.2$obs)

# variable importance 
n <- 130
woody.train <- woody_input2[-n,]
woody.train <- droplevels(woody.train)
woody.test <- woody_input2[n,]
woody.test <- droplevels(woody.test)
...
# MDG
g1 <- ggplot(rankImportance, aes(x = reorder(Variables, Importance), 
...
rankImportance <- varImportance %>%
  mutate(Rank = paste0('#',dense_rank(desc(Importance))))
g2 <- ggplot(rankImportance, aes(x = reorder(Variables, Importance), 
    y = Importance, fill = Variables)) +
  geom_bar(stat='identity') + 
  #geom_text(aes(x = Variables, y = 0.5, label = Rank),
  #  hjust=0, vjust=0.55, size = 4, colour = 'red') +
...
# make histogram and table of remaining species 

# table 
woody_table <- woody_gt %>%
  mutate(sciNameShort = word(scientificName, 1, 2)) %>%
  group_by(taxonID, sciNameShort) %>%
  summarise(total = n()) %>% 
  ungroup() %>%
  inner_join(manu_df, by = "taxonID") %>%
  select(total, taxonID, sciNameShort, commonNames)
kable(woody_table)

# hist 
g <- ggplot(aes(x = taxonID, y = total, fill = taxonID), data = woody_table) + 
  geom_bar(stat="identity") + 
  labs(y = "Total # of samples", x = "Taxon ID") + 
  ggtitle("Number of plants per species") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        panel.background = element_blank(),
        panel.grid.major = element_line(color = "grey10",
                                        ...
                                        size = .1)) 
g
ggsave(g, filename = "SpeciesHist.png", bg = "transparent")

# convert output labels to categorical
woody_gt$taxonID <- factor(woody_gt$taxonID)
woody_gt$growthForm <- factor(woody_gt$growthForm)
woody_gt$plantStatus <- factor(woody_gt$plantStatus)
woody_gt$shape <- factor(woody_gt$shape)
